...
import {
  ...
  addOperationalDays, buildDailyLeaderResult, dailyRankingBonusDocument,
...
(() => {
  ...
  const WEEKLY_PLAQUES_VERSION = "v2.4.26-weekly-selector-tap-sound-manual-hold";
  const WEEKLY_PLAQUES_CACHE_PREFIX = "explora_weekly_plaques_v2415";
  const DAILY_GLOBAL_CACHE_PREFIX = "explora_daily_global_leader_v2412";
  ...
  function loadDailyGlobalCache(dayId = "", weeklyPeriodId = "") {
    ...
    try {
      ...
      const result = computed.hasLeader ? computed : cached.hasLeader ? Object.freeze({
        ...
        status:text(cached.status || "lead"), message:text(cached.message || "LÍDER GLOBAL"),
  ...
  function leaderCardMarkup(result, { compact = false } = {}) {
    if (!result?.hasLeader) {
      return `<article class="daily-leader-card is-empty${compact?" is-compact":""}" data-daily-state="empty"><span class="daily-leader-kicker">LÍDER DEL DÍA</span><p class="daily-leader-empty-copy">Todavía no hay actividad válida hoy.</p></article>`;
    }
    ...
    const leadPercentage = Number(result.winnerLeadPercentage);
    const displayMessage = result.tieBreakApplied
      ? "LIDERA POR DESEMPATE"
      : result.leaderReason === "single_activity" || Number(result.activeDriverCount) === 1
        ? "ÚNICO CHOFER CON ACTIVIDAD HOY"
        : Number.isFinite(leadPercentage)
          ? `${new Intl.NumberFormat("es-AR",{minimumFractionDigits:1,maximumFractionDigits:1}).format(leadPercentage)}% POR DELANTE DEL 2.º`
          : text(result.message || "LÍDER DEL DÍA");
  ...
  function renderDashboard(result = state.result) {
    const signature = [result?.operationalDayId,result?.winnerDriverId,result?.winnerDailyAmount,result?.winnerServiceCount,result?.winnerReachedAtMs,result?.status].join("|");
    atomicRender($("performancePodium"),leaderCardMarkup(result,{compact:true}),signature);
  }
  function renderDetail(result = state.result) {
    const signature = ["detail",result?.operationalDayId,result?.winnerDriverId,result?.winnerDailyAmount,result?.winnerServiceCount,result?.winnerReachedAtMs,result?.status].join("|");
    atomicRender($("performanceRankingList"),leaderCardMarkup(result),signature);
  }
  ...
  function plaqueFromStoredResult(data, definition, todayId) {
    ...
    if (definition.dayId === todayId) return { ...base,state:"current" };
    if (definition.dayId > todayId) return { ...base,state:"locked" };
  ...
  function weekShell(period = currentPeriod(), todayId = operationalDayIdFromDate(operationalNow(),TZ)) {
    ...
    return weekDayDefinitions(period).map(definition => {
      if (definition.dayId === todayId) return { ...definition,state:"current",rankingBonusAmount:0,derivationBonusAmount:0,totalBonusAmount:0 };
      if (definition.dayId > todayId) return { ...definition,state:"locked",rankingBonusAmount:0,derivationBonusAmount:0,totalBonusAmount:0 };
      ...
      return cached ? { ...definition,...cached,weeklyPeriodId:definition.weeklyPeriodId,dayId:definition.dayId,label:definition.label,index:definition.index,source:"local-cache" }
        : { ...definition,state:"no_data",rankingBonusAmount:0,derivationBonusAmount:0,totalBonusAmount:0 };
    });
  }
  ...
  function weeklyCarouselMove(direction = 0, reason = "swipe") {
    ...
    const nextIndex = weeklyCarouselSetIndex(previousIndex + direction, { reason });
    if (isWeeklySelectorManualSoundReason(reason) && nextIndex !== previousIndex) {
  ...
  function bindWeeklyCarousel(viewport, track) {
    ...
    const finish = (x, y) => {
      ...
      if (Math.abs(deltaX) < 28 || Math.abs(deltaX) <= Math.abs(deltaY) * 1.1) return;
    ...
    track.addEventListener("click", event => {
      ...
      if (slot === "left") { weeklyCarouselMove(-1, "tap-left"); return; }
      if (slot === "right") { weeklyCarouselMove(1, "tap-right"); return; }
  ...
  function renderWeeklyPlaques(rows = state.weeklyPlaques, options = {}) {
    ...
    const signature = safeRows.map(row => [row.dayId,row.state,row.winnerDriverId,row.winnerDriverName,row.totalBonusAmount,row.winnerLeadPercentage,row.tieBreakApplied].join(":" )).join("|") + `|center:${centerIndex}`;
    ...
    if (track.dataset.weeklyPlaqueSignature !== signature || track.querySelectorAll("[data-weekly-award-slot]").length !== 3) {
      ...
      template.innerHTML = visible.map(item => weeklyPlaqueMarkup(item.row,item.index,safeRows.length)
        .replace('class="weekly-winner-plaque', `class="weekly-winner-plaque is-carousel-${item.slot}${item.slot === "center" ? " is-carousel-selected" : ""}`)
  ...
  async function readWeeklyPlaqueData(period, todayId) {
    ...
    const storedResults = await Promise.all(definitions.map(async definition => {
      if (definition.dayId >= todayId) return {definition,data:null};
      try {
    ...
    return Promise.all(storedResults.map(async ({definition,data,error}) => {
      if (definition.dayId >= todayId) return plaqueFromStoredResult(data,definition,todayId);
      ...
      const storedPlaque = error
        ...
        : plaqueFromStoredResult(data,definition,todayId);
  ...
  async function refreshWeeklyPlaques({ force = false, background = false } = {}) {
    ...
    const activePromise = (async () => {
      ...
      if (merged.some(row => row.state === "no_data" && row.dayId < todayId)) scheduleWeeklyPlaquesRetry();
      return merged;
    })().catch(error => {
  ...
  function resultFromStoredBonus(data = {}) {
    ...
    const leadText = row.winnerLeadPercentage !== null && row.winnerLeadPercentage !== undefined
      ? new Intl.NumberFormat("es-AR",{minimumFractionDigits:1,maximumFractionDigits:1}).format(row.winnerLeadPercentage)
      : "";
    const message = row.tieBreakApplied
      ? "LIDERA POR DESEMPATE"
      : row.leaderReason === "single_activity"
        ? "ÚNICO CHOFER CON ACTIVIDAD HOY"
        : leadText
          ? `${leadText}% POR DELANTE DEL 2.º`
          : "LÍDER DEFINITIVO DEL DÍA";
  ...
  function applyPublicData(data = {}, expectedDayId = "", expectedPeriodId = "") {
    ...
    if (!dayId || dayId !== expectedDayId || (periodId && expectedPeriodId && periodId !== expectedPeriodId)) {
      if (dayId) diagnostic("VALIDATE_PUBLIC_DAY","DAILY_RANKING_PUBLIC_PERIOD_MISMATCH",new Error("El resumen público recibido pertenece a otro día o período y fue descartado."),{functionName:"applyPublicData",eventType:"WARNING",operationalDayId:expectedDayId,weeklyPeriodId:expectedPeriodId,receivedDayId:dayId,receivedPeriodId:periodId,firestorePath:`${publicCollectionName()}/${dayId}`,query:"getDoc / onSnapshot"});
      return false;
    }
    ...
    const liveDocumentResult = documentResult?.hasLeader ? Object.freeze({
      ...
      message:text(data.message || data.leaderMessage || documentResult.message || "LÍDER GLOBAL DEL DÍA"),
    ...
    const currentOperationalResult = state.dayId === dayId && state.result?.source === "global-operational";
    if (currentOperationalResult) return false;
    state.dayId = dayId; state.weeklyPeriodId = resolvedPeriodId; state.rows = rows; state.result = sourcedDailyResult(result,"daily-public");
    state.lastError = null; saveDailyGlobalCache(dayId,resolvedPeriodId,rows,state.result); renderAll(); validateResultConsistency(state.result,rows,dayId,state.weeklyPeriodId);
    return true;
  }
  ...
  async function readPublicDay(dayId, weeklyPeriodId) {
    if (!db || !dayId) return false;
    try {
      const snapshot = await getDoc(doc(db,publicCollectionName(),dayId));
      return snapshot.exists() ? applyPublicData(snapshot.data() || {},dayId,weeklyPeriodId) : false;
    } catch (error) {
  ...
  function resultFromStoredPublic(data = {}, dayId = "", weeklyPeriodId = "") {
    ...
    return {
      ...
      status:"finalized", message:"Resultado histórico reparado"
    };
  }
  ...
  async function refresh({ force = false } = {}) {
    ...
    state.refreshPromise = (async () => {
      ...
      if (!isAdmin()) {
        ...
        if (cachedGlobal?.result) {
          state.dayId = dayId; state.weeklyPeriodId = period.id; state.records = []; state.rows = cachedGlobal.rows || [];
          state.result = sourcedDailyResult(cachedGlobal.result,"daily-global-cache"); state.lastError = null; renderAll();
        }
        const [publicAttempt,operationalAttempt] = await Promise.allSettled([
          withTimeout(readPublicDay(dayId,period.id),FIRESTORE_READ_TIMEOUT_MS,"DAILY_RANKING_PUBLIC_READ_TIMEOUT"),
          readGlobalOperationalDay(dayId,period.id)
        ]);
        if (requestId !== state.requestId || dayId !== operationalDayIdFromDate(operationalNow(),TZ)) return state.result;
        const publicFound = publicAttempt.status === "fulfilled" && publicAttempt.value === true;
        const publicRowsSnapshot = publicFound ? state.rows.map(row => ({...row})) : [];
        const publicResultSnapshot = publicFound ? state.result : null;
        const globalSnapshot = operationalAttempt.status === "fulfilled" ? operationalAttempt.value : null;
        if (globalSnapshot?.result?.hasLeader) {
          state.dayId = dayId; state.weeklyPeriodId = period.id; state.records = globalSnapshot.records; state.rows = globalSnapshot.rows; state.result = globalSnapshot.result;
        } else if (publicResultSnapshot?.hasLeader) {
          state.dayId = dayId; state.weeklyPeriodId = period.id; state.records = globalSnapshot?.records || []; state.rows = publicRowsSnapshot; state.result = sourcedDailyResult(publicResultSnapshot,"daily-public");
        } else if (globalSnapshot) {
          state.dayId = dayId; state.weeklyPeriodId = period.id; state.records = globalSnapshot.records; state.rows = globalSnapshot.rows; state.result = globalSnapshot.result;
        } else if (cachedGlobal?.result) {
          state.dayId = dayId; state.weeklyPeriodId = period.id; state.records = []; state.rows = cachedGlobal.rows || []; state.result = sourcedDailyResult(cachedGlobal.result,"daily-global-cache");
        } else {
          state.dayId = dayId; state.weeklyPeriodId = period.id; state.records = []; state.rows = []; state.result = neutralDailyResult(dayId,period.id,"no-global-data");
        }
        ...
        renderAll(); validateResultConsistency(state.result,state.rows,dayId,period.id);
      ...
      state.dayId = dayId; state.weeklyPeriodId = period.id; state.records = records; state.rows = rows;
      state.result = buildDailyLeaderResult(rows,dayId,period.id); state.lastError = null;
      renderAll(); validateResultConsistency(state.result,rows,dayId,period.id);
