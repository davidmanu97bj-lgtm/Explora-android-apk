...
(() => {
  ...
  function loadWeeklyPlaquesCache(period, todayId) {
    try {
      ...
      return parsed.rows.filter(row => row && row.dayId && row.dayId < todayId && row.state === "winner");
    } catch (_) { return []; }
  }
  function saveWeeklyPlaquesCache(period, rows = []) {
    try {
      const valid = rows.filter(row => row && row.state === "winner").map(row => ({...row}));
  ...
  function isWeeklyPlaqueFuture(definition = {}, todayId = operationalDayIdFromDate(operationalNow(),TZ), period = currentPeriod()) {
    const dayId = text(definition.dayId);
    return /^\d{4}-\d{2}-\d{2}$/.test(dayId) && /^\d{4}-\d{2}-\d{2}$/.test(todayId)
      ? dayId > todayId
      : weeklyPlaqueTemporalClass(definition,todayId,period) === "future";
  }
  ...
  function isWeeklyPlaqueToday(definition = {}, todayId = operationalDayIdFromDate(operationalNow(),TZ), period = currentPeriod()) {
    ...
    return /^\d{4}-\d{2}-\d{2}$/.test(dayId) && /^\d{4}-\d{2}-\d{2}$/.test(todayId)
      ? dayId === todayId
      : weeklyPlaqueTemporalClass(definition,todayId,period) === "today";
  }
  ...
  function isWeeklyPlaquePast(definition = {}, todayId = operationalDayIdFromDate(operationalNow(),TZ), period = currentPeriod()) {
    ...
    return /^\d{4}-\d{2}-\d{2}$/.test(dayId) && /^\d{4}-\d{2}-\d{2}$/.test(todayId)
      ? dayId < todayId
      : weeklyPlaqueTemporalClass(definition,todayId,period) === "past";
  }
    ...
    return weekDayDefinitions(period).map(definition => {
      ...
      if (cached) {
        return {
          ...definition,
          ...cached,
          weeklyPeriodId: definition.weeklyPeriodId,
      ...
      // Nunca mostrar un estado transitorio "Sin datos" en las placas.
      return {
        ...definition,
        state: "locked",
        rankingBonusAmount: 0,
        derivationBonusAmount: 0,
        totalBonusAmount: 0,
      };
    });
  }
  ...
  function weeklyPlaqueMarkup(row = {}, index = 0, total = 7) {
    ...
    return `<button type="button" aria-disabled="true" class="weekly-winner-plaque is-locked" ${common} aria-label="${escapeHtml(`${row.label}, bloqueado mientras se confirma el ganador`)}"><span class="weekly-plaque-lock" aria-hidden="true"></span><span class="weekly-plaque-day">${escapeHtml(row.label)}</span><span class="weekly-plaque-badge">BLOQUEADO</span><strong class="weekly-plaque-primary">PRÓXIMO DÍA</strong><span class="weekly-plaque-total-label">PREMIO DISPONIBLE</span><b class="weekly-plaque-total">${escapeHtml(money(DAILY_RANKING_BONUS_AMOUNT))}</b><small class="weekly-plaque-note">Ranking diario</small></button>`;
  }
  ...
  async function refreshWeeklyPlaques({ force = false, background = false } = {}) {
    ...
    const activePromise = (async () => {
      ...
      const merged = rows.map(row => {
        ...
        return previous?.state === "winner" ? previous : {...row,state:"locked"};
      });
